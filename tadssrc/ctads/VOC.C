#ifdef RCSID
static char RCSid[] =
"$Header: d:/tads/tads2/RCS/voc.c 1.8 96/10/14 16:10:50 mroberts Exp $";
#endif
...
static void voccpy(dst, src, len)
char *dst;
char *src;
int   len;
{
...
void vocwset(ctx, v, p, objn, classflg)
voccxdef *ctx;
vocdef   *v;
prpnum    p;
objnum    objn;
int       classflg;
{
	...
	{
	    if (vw2 != vw && vw2->vocwtyp == PRP_VERB)
	    {
		errlog(ctx->voccxerr, ERR_VOCREVB);
		break;
	    }
	}
    }
...
void vocset(ctx, v, p, objn, classflg, wrdtxt, len, wrd2, len2)
voccxdef *ctx;
vocdef   *v;
prpnum    p;
objnum    objn;
...
void vocadd2(ctx, p, objn, classflg, wrdtxt, len, wrd2, len2)
voccxdef *ctx;
prpnum    p;
objnum    objn;
int       classflg;
...
void vocadd(ctx, p, objn, classflg, wrdtxt)
voccxdef *ctx;
prpnum    p;
objnum    objn;
int       classflg;
...
void vociadd(ctx, obj, loc, numsc, sc, flags)
voccxdef *ctx;
objnum    obj;
objnum    loc;
int       numsc;
...
void vocini(vocctx, errctx, memctx, runctx, undoctx, fuses, daemons,
	    notifiers)
voccxdef  *vocctx;
errcxdef  *errctx;
mcmcxdef  *memctx;
 ...
 */
void voc_iterate(ctx, objn, fn, fnctx)
voccxdef  *ctx;
objnum     objn;
void     (*fn)(/*_ void *fnctx, vocdef *voc, vocwdef *vocw _*/);
void      *fnctx;
{
    int       i;
    vocdef   *v;
...
static void voc_count_cb(ctx, voc, vocw)
struct voc_count_ctx *ctx;
vocdef               *voc;
vocwdef              *vocw;
{
    VARUSED(vocw);
 ...
 */
void voc_count(ctx, objn, prp, cnt, siz)
voccxdef *ctx;
objnum    objn;
prpnum    prp;
int      *cnt;
 ...
 */
void vocdel1(ctx, objn, wrd1, prp, really_delete, revert, keep_undo)
voccxdef *ctx;
objnum    objn;
char     *wrd1;
prpnum    prp;
...
int       keep_undo;
{
	...
	for (prv = (vocdef *)0, v = *vp ; v ; v = nxt)
	{
		...
		if (v->voclen != len1 || memicmp((char *)v->voctxt, wrd1, len1)) //###cast
		    continue;

		/* if there's a second word, compare it as well */
		if (wrd2 && (v->vocln2 != len2
			     || memicmp((char *)v->voctxt + len1, wrd2, len2))) //###cast
		    continue;
	    }
 ...
 */
int voctplfnd(ctx, verb_in, prep, tplout, newstyle)
voccxdef *ctx;
objnum    verb_in;
objnum    prep;
uchar    *tplout;                       /* output buffer for found template */
int      *newstyle;
{
    uchar  *tplptr;
    uchar  *thistpl;
