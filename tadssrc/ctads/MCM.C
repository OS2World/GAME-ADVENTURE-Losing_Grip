...
static mcmodef *mcmoal(/*_ mcmcx1def *ctx, mcmon *objnum _*/);

/* split a (previously free) block into two pieces */
static void mcmsplt(/*_ mcmcx1def *ctx, mcmon n, ushort siz _*/);

/* unlink an object from a doubly-linked list */
static void mcmunl(/*_ mcmcx1def *ctx, mcmon n, mcmon *lst _*/);

/* initialize a cache, return cache context */
/* find free block: find a block from the free pool to satisfy a request */
static mcmodef *mcmffb(/*_ mcmcx1def *ctx, ushort siz, mcmon *nump _*/);

/* add page pagenum, initializing entries after firstunu to unused */
static void mcmadpg(/*_ mcmcx1def *ctx, uint pagenum, mcmon firstunu _*/);

/* link an object into a doubly-linked list at the head of the list */
static void mcmlnkhd(/*_ mcmcx1def *ctx, mcmon *lst, mcmon n _*/);

/* try to allocate a new chunk from the heap */
static uchar *mcmhalo(/*_ mcmcx1def *ctx _*/);

/* relocate blocks in a heap */
static uchar *mcmreloc(/*_ mcmcx1def *ctx, uchar *start, uchar *end _*/);

/* find next free heap block */
static uchar *mcmffh(/*_ mcmcx1def *ctx, uchar *p _*/);

#ifdef NEVER
/* update entry to account for a block relocation */
static void mcmmove(/*_ mcmcx1def *ctx, mcmodef *obj, uchar *newaddr _*/);
#else /* NEVER */
#define mcmmove(ctx, o, new) ((o)->mcmoptr = (new))
#endif /* NEVER */

/* consolidate two contiguous free blocks into a single block */
static void mcmconsol(/*_ mcmcx1def *ctx, uchar *p _*/);

/* collect garbage in all heaps */
static void mcmgarb(/*_ mcmcx1def *ctx _*/);

/* make some room by swapping or discarding objects */
static int mcmswap(/*_ mcmcx1def *ctx, ushort siz _*/);

/* toss out an object; returns TRUE if successful */
static int mcmtoss(/*_ mcmcx1def *ctx, mcmon objnum _*/);
...
mcmcxdef *mcmcini(globalctx, pages, loadfn, loadctx, revertfn, revertctx)
mcmcx1def *globalctx;
uint       pages;
void     (*loadfn)(/*_ dvoid *ctx, mclhd handle, uchar *ptr, ushort siz _*/);
dvoid     *loadctx;
void     (*revertfn)(/*_ dvoid *ctx, mcmon objn _*/);
dvoid     *revertctx;
{
    mcmcxdef *ret;
    ushort    siz;
...
mcmcx1def *mcmini(max, pages, swapsize, swapfp, errctx)
ulong     max;            /* maximum amount of heap we should ever allocate */
uint      pages;                   /* maximum number of pages in page table */
ulong     swapsize;                            /* maximum size of swap file */
osfildef *swapfp; //###indep                     /* file to use for swapping */
errcxdef *errctx;                                 /* error handling context */
{
    ...
    ERRBEGIN(errctx)
        mcsini(&ctx->mcmcxswc, ctx, swapsize, swapfp, errctx);
    ERRCATCH(errctx, err)
 ...
 *   is returned at *nump.
 */
static uchar *mcmalo1(ctx, siz, nump)
mcmcx1def *ctx;                                           /* global context */
ushort     siz;
mcmon     *nump;                  /* place to return a GLOBAL object number */
{
    mcmon    n;
    mcmodef *o;
    uchar   *chunk;
...
static void mcmcliexp(cctx, clinum)
mcmcxdef *cctx;
mcmon     clinum;
{
...
uchar *mcmalo0(cctx, siz, nump, clinum, noclitrans)
mcmcxdef *cctx;
ushort    siz;
mcmon    *nump;
mcmon     clinum;  /* client number, if requested; MCMONINV to generate one */
int       noclitrans;             /* TRUE ==> leave as GLOBAL object number */
{
    uchar     *ret;
    ...
    if (ret = mcmalo1(ctx, siz, &glb)) goto done;               /* try once */
    mcmgarb(ctx);                                   /* collect some garbage */
    
    /* try swapping until we get the memory or have nothing left to swap */
    for ( ;; )
    {
        if (ret = mcmalo1(ctx, siz, &glb)) goto done;          /* try again */
        if (!mcmswap(ctx, siz)) break;              /* nothing left to swap */
        if (ret = mcmalo1(ctx, siz, &glb)) goto done;          /* try again */
        mcmgarb(ctx);                         /* collect garbage once again */
    }
...
void mcmrsrv(cctx, siz, clinum, loadhd)
mcmcxdef *cctx;
ushort    siz;
mcmon     clinum;
mclhd     loadhd;
...
uchar *mcmrealo(cctx, cliobj, newsize)
mcmcxdef *cctx;
mcmon     cliobj;
ushort    newsize;
{
    ...
    if (local_lock = !(o->mcmoflg & MCMOFLOCK)) (void)mcmlck(cctx, cliobj);
...
mcmon     cnum;
{
    ...
    new = mcmgobje(ctx, newn);
    
    /* use memory block from our new object */
    o->mcmoptr = new->mcmoptr;
    o->mcmosiz = new->mcmosiz;
...
/* @@@ */
    *(mcmon *)(o->mcmoptr - osrndsz(sizeof(mcmon))) = num;      /* set obj# */
    new->mcmoflg = 0;                           /* mark new block as unused */
    new->mcmonxt = ctx->mcmcxunu;                   /* link to unused chain */
    ctx->mcmcxunu = newn;
 ...
 *   the header for one. 
 */
static mcmodef *mcmoal(ctx, nump)
mcmcx1def *ctx;
mcmon     *nump;
{
    mcmodef  *ret;
...
static mcmodef *mcmffb(ctx, siz, nump)
mcmcx1def *ctx;
ushort     siz;
mcmon     *nump;
{
    ...
    for (minn = MCMONINV, n = ctx->mcmcxfre ; n != MCMONINV ; n = o->mcmonxt)
    {
        o = mcmgobje(ctx, n);
        if (o->mcmosiz == siz)
        {
...
static void mcmlnkhd(ctx, lst, n)
mcmcx1def *ctx;
mcmon     *lst;
mcmon      n;
{
...
/* add page pagenum, initializing entries after firstunu to unused */
static void mcmadpg(ctx, pagenum, firstunu)
mcmcx1def *ctx;
uint       pagenum;
mcmon      firstunu;  /* first unused entry in page (MCMONINV if none used) */
{
    mcmon    unu;
    mcmodef *obj;
    mcmon    lastunu;
 ...
 */ 
static void mcmsplt(ctx, n, siz)
mcmcx1def *ctx;
mcmon      n;                     /* the newly-allocated object to be split */
ushort     siz;        /* size of newly allocated part to remove from block */
{
    mcmodef *o = mcmgobje(ctx, n);
    mcmon    newn;
    mcmodef *newp;
...
uchar *mcmlck(ctx, objnum)
mcmcxdef *ctx;
mcmon     objnum;
{
    mcmodef *o = mcmobje(ctx, objnum);
    
    if (o->mcmoflg & MCMOFPRES)
    {
        o->mcmoflg |= MCMOFLOCK;
        ++(o->mcmolcnt);
        return(o->mcmoptr);
 ...
 */
void mcmcswf(ctx, objn, swapn, oldswapn)
mcmcx1def *ctx;
ushort     objn;
mcsseg     swapn;
mcsseg     oldswapn;
