#ifdef RCSID
static char RCSid[] =
"$Header: c:/tads/tads2/RCS/OBJ.C 1.5 94/11/06 13:07:19 mroberts Exp $";
#endif
 ...
 *   found.
 */
uint objgetp(mctx, objn, prop, typptr)
mcmcxdef *mctx;
objnum    objn;
prpnum    prop;
dattyp   *typptr;
{
	...
	/* ignore ignored properties if possible */
	while (retval && prpflg(objptr + retval) & PRPFIGN
...
static int objisd(ctx, objptr, parentnum)
mcmcxdef *ctx;
objdef   *objptr;
objnum    parentnum;
{
    ...
    for (sc = (uchar *)objsc(objptr), cnt = objnsc(objptr) ; cnt ;
         sc += 2, --cnt)
    {
	int     cursc = osrp2(sc);
	int     ret;
	...
	curptr = (objdef *)mcmlck(ctx, cursc);
 ...
 */
uint objgetap(ctx, obj, prop, orn, inh)
mcmcxdef       *ctx;                               /* cache manager context */
noreg objnum    obj;                         /* object number of the object */
prpnum          prop;                                   /* property to find */
objnum         *orn;   /* number of obj where prop is found is written here */
int             inh;                        /* flag:  TRUE ==> inherit ONLY */
{
    objdef *objptr;                             /* pointer to locked object */
    prpdef *p;
    uchar   *sc; //###uchar
    ushort  sccnt;
    ushort  psav;
...
startover:
    ...
    if (!inh && (retval = objgetp(ctx, obj, prop, &typ)))
    {
	/* check to see if synonym translation is required */
	if (typ == DAT_SYN)
	{
    ...
    for (found = FALSE ; sccnt ; sc += 2, --sccnt)
    {
	/* recursively look up the property in this superclass */
	poff = objgetap(ctx, osrp2(sc), prop, &o1n, FALSE);
 ...
 *   clearing the 'ignore' flag on the remaining properties. 
 */
void objdelp(mctx, objn, prop)
mcmcxdef *mctx;
objnum    objn;
prpnum    prop;
{
 ...
 */
void objsetp(ctx, objn, prop, typ, val, undoctx)
mcmcxdef  *ctx;
objnum     objn;
prpnum     prop;
dattyp     typ;
...
objucxdef *undoctx;
{
	 ...
	 */
	if (!p || prpsize(p) < siz || pofs < objrst(objptr))
	{
	    uint   avail;
	    size_t indsiz;
	    
	    if (prop_was_set) objdelp(ctx, objn, prop);    /* del old value */
	    p = objpfre(objptr);        /* get the top of the property area */
...
uchar *objures(undoctx, cmd, siz)
objucxdef *undoctx;                                         /* undo context */
uchar      cmd;                                             /* undo command */
ushort     siz;                        /* size of extra space beyond header */
{
    ushort  prv;
    uchar  *p;
...
objucxdef *undoctx;
{
	...
	p += sizeof(prop);
	objptr = (objdef *)mcmlck(mctx, objn); //###cast
	indexed = (objflg(objptr) & OBJFINDEX);
	mcmunlck(mctx, objn);
    }
    ...
    {
    case OBJUADD:
	objdelp(mctx, objn, prop);
	if (indexed) objindx(mctx, objn);
	break;
	
    case OBJUOVR:
	objdelp(mctx, objn, prop);         /* delete the non-original value */
	...
	p += PRPHDRSIZ;
	objsetp(mctx, objn, prop, prptype(&pr), (dvoid *)p, (objucxdef *)0);
	break;
...
objucxdef *objuini(ctx, siz, undocb, sizecb, callctx)
mcmcxdef *ctx;
ushort    siz;                                       /* size of undo buffer */
void    (*undocb)(/*_ dvoid *ctx, uchar *data _*/);       /* client undo fn */
ushort  (*sizecb)(/*_ dvoid *ctx, uchar *data _*/); /* client size callback */
dvoid    *callctx;                           /* context for client callback */
{
    objucxdef *ret;
    long       totsiz;
...
objdef *objnew(mctx, sccnt, propspace, objnptr, class)
mcmcxdef *mctx;
int       sccnt;
ushort    propspace;
objnum   *objnptr;
int       class;
{
    ...
    objini(mctx, sccnt, (objnum)objn, class);
...
void objini(mctx, sccnt, objn, class)
mcmcxdef *mctx;
int       sccnt;
objnum    objn;
int       class;
...
objnum    objn;
{
    ...
    /* lock the object */
    p = (objdef *)mcmlck(ctx, (mcmon)objn); //###cast
