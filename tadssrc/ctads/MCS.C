#ifdef RCSID
static char RCSid[] =
"$Header: c:/tads/tads2/RCS/MCS.C 1.4 94/11/06 13:05:31 mroberts Exp $";
#endif
...
void mcsini(ctx, gmemctx, maxsiz, fp, errctx)
mcscxdef  *ctx;
mcmcx1def *gmemctx;
ulong      maxsiz;
osfildef  *fp; //###indep
...
mcscxdef *ctx;
{
	    ...
	    && (desc_in->mcsdsflg & MCSDSFINUSE))
	{
		...
		osfseek(ctx->mcscxfp, ptr_in, OSFSK_SET); //###indep
		osfrb(ctx->mcscxfp, buf, (size_t)rdsiz); //###indep
		
		/* seek to new location and write the piece */
		osfseek(ctx->mcscxfp, ptr_out, OSFSK_SET); //###indep
		osfwb(ctx->mcscxfp, buf, (size_t)rdsiz); //###indep
	...
	if (desc_in->mcsdsflg & MCSDSFINUSE)
	{
		...
		mcmcsw(ctx->mcscxmem, desc_in->mcsdsobj, cur_out, cur_in);
	    }
	...
	else
	{
	     ...
	     */
	    mcmcsw(ctx->mcscxmem, desc_in->mcsdsobj, MCSSEGINV, cur_in);
	}
    }
...
mcsseg mcsout(ctx, objid, ptr, siz, oldseg, dirty)
mcscxdef *ctx;
uint      objid;
uchar    *ptr;
ushort    siz;
...
ushort    siz;
{
    ...
    if (osfseek(ctx->mcscxfp, desc->mcsdsptr, OSFSK_SET)) //###indep
		errsig(ctx->mcscxerr, ERR_FSEEK);
    if (osfrb(ctx->mcscxfp, ptr, (size_t)siz)) //###indep
		errsig(ctx->mcscxerr, ERR_FREAD);
    
    desc->mcsdsflg &= ~MCSDSFINUSE;             /* segment no longer in use */
}

void mcswrt(ctx, desc, buf, bufl)
mcscxdef *ctx;
mcsdsdef *desc;
uchar    *buf;
ushort    bufl;
{
	...
	if (osfseek(ctx->mcscxfp, desc->mcsdsptr, OSFSK_SET)) //###indep
	    errsig(ctx->mcscxerr, ERR_FSEEK);
	if (osfwb(ctx->mcscxfp, buf, (size_t)bufl) == 0) //###indep
	    return;
